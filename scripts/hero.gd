...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("grab"):
		if held_object: #solo entra si ya se tiene el objeto tomado
			...
			held_object.collision_mask=3 # Mask 1 y 2
			...
			if !(_objects.empty()):
				...
				held_object.collision_mask=0 # Mask 1  y 2
	...
	if Input.is_action_just_pressed("throw") and is_on_floor() and held_object:
		...
		held_object.take_damage(self) #Primero aplicar el impuslo sin colisionar con el hero
		held_object.collision_mask=3  #Luego cambiar la mask para que no atraviese piso o paredes
		held_object =  null
